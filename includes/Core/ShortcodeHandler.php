...
class ShortcodeHandler {
	...
	public function register() {
		...
		add_shortcode( 'nobat_new', array( $this, 'booking_new_form' ) );
	}
	...
	/**
	 * Booking new form shortcode handler
	 *
	 * @param array $atts Shortcode attributes.
	 * @return string HTML output.
	 */
	public function booking_new_form( $atts ) {
		// Parse shortcode attributes
		$atts = shortcode_atts( array(
			'schedule_id' => '',
		), $atts );
		...
		// Add additional localization data for this shortcode instance
		// Note: The script is already enqueued by enqueue-scripts.php
		$this->localize_booking_new_data();

		// Generate unique container ID
		$container_id = 'nobat-new-' . uniqid();
	...
	/**
	 * Add localization data for booking new form
	 * The script itself is enqueued by enqueue-scripts.php
	 */
	private function localize_booking_new_data() {
		// Only localize if not already done
		static $localized = false;
		if ( $localized ) {
			return;
		...
		// Add additional booking-new-specific data
		wp_localize_script(
			'nobat-front-script', // Use the same handle as in enqueue-scripts.php
			'nobatBookingNew',
			array(
				'apiUrl' => rest_url( 'nobat/v2' ),
				'nonce' => wp_create_nonce( 'wp_rest' ),
